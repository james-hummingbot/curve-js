...
async function getCryptoFactoryTokenAddresses(
  this: ICurve,
  factorySwapAddresses: string[]
): Promise<string[]> {
...
async function getCryptoFactoryGaugeAddresses(
  this: ICurve,
  factorySwapAddresses: string[]
): Promise<string[]> {
  const factoryMulticallContract = await this.contracts[
...
async function getCryptoFactorySymbolsAndNames(
  this: ICurve,
  factoryTokenAddresses: string[]
): Promise<[string[], string[]]> {
  const calls = [];
...
async function getCryptoFactoryCoinAddresses(
  this: ICurve,
  factorySwapAddresses: string[]
): Promise<string[][]> {
  const factoryMulticallContract = await this.contracts[
...
async function getCryptoFactoryUnderlyingCoinAddresses(
  this: ICurve,
  coinAddresses: string[][]
): Promise<string[][]> {
  return coinAddresses.map((coins: string[]) =>
    coins.map((c) =>
      c === WETH_ADDRESS ? '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' : c
    )
  );
}
...
export async function getCryptoFactoryPoolData(
  ...
  const gaugeAddresses = await getCryptoFactoryGaugeAddresses.call(
    this,
    swapAddresses
  );
  ...
  const [poolSymbols, poolNames] = await getCryptoFactorySymbolsAndNames.call(
    this,
    tokenAddresses
  );
  const coinAddresses = await getCryptoFactoryCoinAddresses.call(
    this,
    swapAddresses
  );
  setCryptoFactoryCoinsContracts.call(this, coinAddresses);
  const underlyingCoinAddresses =
    await getCryptoFactoryUnderlyingCoinAddresses.call(this, coinAddresses);
