...
export async function getFactoryPoolsDataFromApi(
  ...
  // Filter duplications
  const mainAddresses = Object.values(
  ...
  rawPoolList = rawPoolList.filter(
    (p) => !mainAddresses.includes(p.address.toLowerCase())
  );
  ...
  rawPoolList.forEach((pool) => {
    ...
    if (isCrypto) {
      const cryptoCoinNames = pool.coins.map((c) =>
        c.symbol === 'ETH' ? 'WETH' : c.symbol
      );
      const underlyingCoinNames = pool.coins.map((c) =>
        c.symbol === 'WETH' ? 'ETH' : c.symbol
      );
      const underlyingCoinAddresses = pool.coins.map((c) =>
        c.address.toLowerCase() === WETH_ADDRESS
          ? ETH_ADDRESS
      ...
      const implementationABIDict =
        this.chainId === 137
          ? implementationABIDictPolygon
          : implementationABIDictEthereum;
      const implementationBasePoolAddressDict =
        this.chainId === 137
          ? implementationBasePoolAddressDictPolygon
          : implementationBasePoolAddressDictEthereum;
      ...
      const basePoolAddress =
        implementationBasePoolAddressDict[pool.implementationAddress];
      const basePoolCoinNames = basePoolAddressCoinsDict[basePoolAddress];
      const basePoolCoinAddresses =
        basePoolAddressCoinAddressesDict[basePoolAddress];
