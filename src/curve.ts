...
class Curve implements ICurve {
  ...
  async init(
    ...
    if (providerType.toLowerCase() === 'JsonRpc'.toLowerCase()) {
      ...
      if (providerSettings.url) {
        this.provider = this.provider =
          new ethers.providers.StaticJsonRpcProvider(providerSettings.url);
      } else {
    ...
    const network =
      this.provider.network || (await this.provider._networkPromise);
    ...
    if (network.chainId === 1 || network.chainId === 1337) {
      ...
      throw Error(
        `Network with chainId ${this.provider.network.chainId} is not supported`
      );
    }

    LP_TOKENS = Object.values(POOLS_DATA).map((data) =>
      data.token_address.toLowerCase()
    );
    GAUGES = Object.values(POOLS_DATA).map((data) =>
      data.gauge_address.toLowerCase()
    );
    ...
    for (const pool of Object.values(POOLS_DATA)) {
      ...
      if (
        ...
        this.contracts[pool.deposit_address] === undefined
      ) {
    ...
    const addressProviderContract =
      this.contracts[ALIASES.address_provider].contract;
    ALIASES.registry_exchange = await addressProviderContract.get_address(
      2,
  ...
  async fetchFactoryPools(useApi = true): Promise<void> {
    if (useApi) {
      this.constants.FACTORY_POOLS_DATA = await getFactoryPoolsDataFromApi.call(
        this,
        false
      );
    } else {
