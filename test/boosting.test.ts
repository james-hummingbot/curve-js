...
describe('Boosting', function () {
  ...
  it('Creates lock in Voting Escrow contract', async function () {
    ...
    const { lockedAmount, unlockTime } =
      (await getLockedAmountAndUnlockTime()) as {
        lockedAmount: string;
        unlockTime: number;
      };
    ...
    assert.deepEqual(
      BN(lockedAmount),
      BN(initialCrvBalance).minus(BN(crvBalance))
    );
    assert.isAtLeast(
      unlockTime + 7 * 86400 * 1000,
      lockTime + 365 * 86400 * 1000
    );
  });
  ...
  it('Increases amount locked in Voting Escrow contract', async function () {
    ...
    const initialCrvBalance: string = (await getCrv()) as string;
    const { lockedAmount: initialLockedAmount } =
      (await getLockedAmountAndUnlockTime()) as {
        lockedAmount: string;
        unlockTime: number;
      };
    ...
    const crvBalance = (await getCrv()) as string;
    const { lockedAmount } = (await getLockedAmountAndUnlockTime()) as {
      lockedAmount: string;
      unlockTime: number;
    };

    assert.deepEqual(
      BN(lockedAmount).minus(BN(initialLockedAmount)),
      BN(initialCrvBalance).minus(BN(crvBalance))
    );
  });

  it('Extends lock time', async function () {
    const { unlockTime: initialUnlockTime } =
      (await getLockedAmountAndUnlockTime()) as {
        lockedAmount: string;
        unlockTime: number;
      };
    await increaseUnlockTime(120);
    const { unlockTime } = (await getLockedAmountAndUnlockTime(address)) as {
      lockedAmount: string;
      unlockTime: number;
    };
    ...
    assert.isAtLeast(
      unlockTime + 7 * 86400 * 1000,
      initialUnlockTime + 120 * 86400 * 1000
    );
  });
});
