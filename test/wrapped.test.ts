  ...
  describe(`${id} add/remove liquidity`, function () {
    ...
    it('Adds liquidity', async function () {
      ...
      pool.coins.forEach((c: string) => {
        if (
          ['aave', 'saave'].includes(id) ||
          (curve.chainId === 137 && pool.id === 'ren')
        ) {
          // Because of increasing quantity
          assert.approximately(
            Number(BN(balances[c])),
            Number(BN(initialBalances[c]).minus(BN(amount).toString())),
            1e-2
          );
          ...
          assert.deepStrictEqual(
            BN(balances[c]),
            BN(initialBalances[c]).minus(BN(amount))
          );
        }
      });
    ...
    it('Removes liquidity', async function () {
      ...
      const lpTokenAmount: string = BN(initialBalances.lpToken)
        .div(10)
        .toFixed(18);
      const coinsExpected = await pool.removeLiquidityWrappedExpected(
        lpTokenAmount
      );
    ...
    it('Removes liquidity imbalance', async function () {
      if (pool.isCrypto) {
        ...
        const lpTokenExpected =
          await pool.removeLiquidityImbalanceWrappedExpected(amounts);
        ...
        pool.coins.forEach((c: string) => {
          if (
            ...
            assert.approximately(
              ...
              Number(BN(balances[c]).minus(BN(amount)).toString()),
              1e-4
            );
          } else {
            assert.deepStrictEqual(
              BN(initialBalances[c]),
              BN(balances[c]).minus(BN(amount))
            );
          }
        });
      }
    ...
    if (!['compound', 'usdt', 'y', 'busd', 'pax'].includes(id)) {
      it('Removes liquidity one coin', async function () {
        ...
        const lpTokenAmount: string = BN(initialBalances.lpToken)
          .div(10)
          .toFixed(18);
        const expected = await pool.removeLiquidityOneCoinWrappedExpected(
          lpTokenAmount,
          0
        );
          ...
          it(`${i} --> ${j}`, async function () {
            ...
            if (i >= coinAddresses.length || j >= coinAddresses.length) {
              ...
              const initialCoinBalances =
                (await pool.coinBalances()) as DictInterface<string>;
              const expected = await pool.exchangeWrappedExpected(
                i,
                j,
                swapAmount
              );
