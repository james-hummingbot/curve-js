...
const balancesTest = async () => {
  ...
  console.log(
    await saave.balances('0x0063046686E46Dc6F15918b61AE2B121458534a5')
  );
  // Or several addresses
  console.log(
    await saave.balances(
...
const poolTest = async () => {
  await curve.init(
    'JsonRpc',
    { url: 'http://localhost:8545/', privateKey: '' },
  ...
  const expectedLpTokenAmount1 = await pool.addLiquidityExpected([
    '100',
    '100',
    '100',
  ]);
  ...
  const expectedLpTokenAmount2 = await pool.addLiquidityWrappedExpected([
    '100',
    '100',
    '100',
  ]);
  ...
  const expectedUnderlyingCoinAmounts = await pool.removeLiquidityExpected(
    '10'
  );
  ...
  const expectedLpTokenAmount3 = await pool.removeLiquidityImbalanceExpected([
    '10',
    '10',
    '10',
  ]);
  console.log(expectedLpTokenAmount3);
  const removeLiquidityImbalanceTx = await pool.removeLiquidityImbalance([
    '10',
    '10',
    '10',
  ]);
  ...
  const expectedDAIAmount = await pool.removeLiquidityOneCoinExpected(
    '10',
    'DAI'
  );
  // OR const expectedDAIAmount = await pool.removeLiquidityOneCoinExpected('10', 0);
  console.log(expectedDAIAmount);
  const removeLiquidityOneCoinTx = await pool.removeLiquidityOneCoin(
    '10',
    'DAI'
  );
  ...
  const expectedADAIAmount = await pool.removeLiquidityOneCoinWrappedExpected(
    '10',
    'aUSDC'
  );
  // OR const expectedADAIAmount = await pool.removeLiquidityOneCoinWrappedExpected('10', 1);
  console.log(expectedADAIAmount);
  const removeLiquidityOneCoinWrappedTx =
    await pool.removeLiquidityOneCoinWrapped('10', 'aUSDC');
...
const exchangeTest = async () => {
  ...
  const { poolAddress, output } = await curve.getBestPoolAndOutput(
    'DAI',
    'USDC',
    '100'
  );
...
const crossAssetExchangeTest = async () => {
  ...
  console.log(
    await curve.crossAssetExchangeOutputAndSlippage('DAI', 'WBTC', '500')
  );
